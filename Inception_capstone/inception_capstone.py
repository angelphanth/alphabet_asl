...
pretrained = InceptionResNetV2(weights='imagenet', include_top=False, pooling='max', input_shape=(pixels,pixels,channels))

# Display the model
pretrained.summary()

# setting them to not trainable so weights wont be tweaked
for layer in pretrained.layers[:195]:
...
plot_model(model, to_file='inception_resnet_capstone.png', 
...
checkpoint = ModelCheckpoint("best_weights_inception_capstone.h5", monitor='val_accuracy', verbose=1, save_best_only=True, mode='max')
...
with open("inception_capstone.json","w") as json_file:
...
bestmodel.load_weights("best_weights_inception_capstone.h5")
