...
### REQUIREMENTS ###
#   - Download and unzip the ASL dataset from: https://www.kaggle.com/grassknoted/asl-alphabet
#   - Move the inner-most asl_alphabet_train directory into: alphabet_asl (this git repo)
...
#   - Run a python script that will create brightened grey and colour versions of the images (Option here to save the dataframes of the image arrays to .csv files) 
...
mkdir -p "../asl_list/$1"
# Sanity verbose
echo "A new directory '../$1' was created."
echo "A new directory '../$1' was created." >> "../asl_list/$1/$2"

# Creating a list of the class filenames to iterate through
ls "../asl_alphabet_train/training_set" > "../asl_list/$1/$3"
# Sanity verbose
echo "A list of classes was saved to 'asl_list/$1/$3'."
echo "A list of classes was saved to 'asl_list/$1/$3'." >> "../asl_list/$1/$2"
...
do 
    ...
    mkdir -p ../asl_alphabet_train/{training_set/"$letter",test_set/"$letter"}
    mkdir -p ../brighten_only/{training_set/"$letter",test_set/"$letter"}
    mkdir -p ../bright_grey/{training_set/"$letter",test_set/"$letter"}
    # Sanity verbose
    echo "Directory '$letter/' was created in 'training_set/' and 'test_set/'."
    echo "Directory '$letter/' was created in 'training_set/' and 'test_set/'." >> "../asl_list/$1/$2"

    # A count of training images in that class
    num_images=$(ls -1q "../asl_alphabet_train/training_set/$letter" | wc -l)
    # Calculate 20% of num_images
    test_size=$(expr $num_images / 5)
  
    # Saving the list of random images to a text file
    ls "../asl_alphabet_train/training_set/$letter" | sort -R | tail -$test_size > "../asl_list/$1/test_$letter.txt"
    # Sanity verbose 
    echo "The 'asl_list/$1/test_$letter.txt' was created."
    echo "The 'asl_list/$1/test_$letter.txt' was created." >> "../asl_list/$1/$2"

    # Also saving the test images with prefices 'bright_' and 'grey_' that will later be iterated through in the shell
    sed -e 's/^/bright_/' "../asl_list/$1/test_$letter.txt" > "../asl_list/$1/bright_test_$letter.txt"
    sed -e 's/^/grey_/' "../asl_list/$1/test_$letter.txt" > "../asl_list/$1/grey_test_$letter.txt"
    # Sanity verbose 
    echo "'asl_list/$1/bright_test_$letter.txt grey_test_$letter.txt' were created."
    echo "'asl_list/$1/bright_test_$letter.txt grey_test_$letter.txt' were created." >> "../asl_list/$1/$2"

done < "../asl_list/$1/$3"

# Update
echo "Moving on to brightening the images..."
echo "Moving on to brightening the images..." >> "../asl_list/$1/$2"

# Execute python script to created brightened images (grey and colour versions) and dataframes for future MLClassifiers
python "../bright_images.py" "$1" "$3" asl_grey.csv asl_colour.csv asl_original.csv

# Update 
echo "Brightened Grey and Colour Images and their dataframes (in .../asl_list/$1/) have been created."
echo "Brightened Grey and Colour Images and their dataframes (in .../asl_list/$1/) have been created." >> "../asl_list/$1/$2"
...
do
    ...
    do
        ...
        mv "../asl_alphabet_train/training_set/$letter/$image" "../asl_alphabet_train/test_set/$letter/"

        # Sanity verbose
        echo "$image was moved to test_set/$letter"
        echo "$image was moved to test_set/$letter" >> "../asl_list/$1/$2"

    done < "../asl_list/$1/test_$letter.txt"

    # Move brightened, grey images in 'grey_test_{letter}.txt' to bright_grey/test_set/
    while read grey_img 
    do
        # Move the images listed in testlist to test_set in designated class
        mv "../bright_grey/training_set/$letter/$grey_img" "../bright_grey/test_set/$letter/"

        # Sanity verbose
        echo "$grey_img was moved to test_set/$letter"
        echo "$grey_img was moved to test_set/$letter" >> "../asl_list/$1/$2"

    done < "../asl_list/$1/grey_test_$letter.txt"

    # Move brightened, colour images in 'bright_test_{letter}.txt' to brighten_only/test_set/ 
    while read colour_img 
    do
        # Move the images listed in testlist to test_set in designated class
        mv "../brighten_only/training_set/$letter/$colour_img" "../brighten_only/test_set/$letter/"

        # Sanity verbose
        echo "$colour_img was moved to test_set/$letter"
        echo "$colour_img was moved to test_set/$letter" >> "../asl_list/$1/$2"

    done < "../asl_list/$1/bright_test_$letter.txt"

    # Update
    echo "Class {$letter} test sets made for original, brightened grey and brightened colour images."
    echo "Class {$letter} test sets made for original, brightened grey and brightened colour images." >> "../asl_list/$1/$2"
